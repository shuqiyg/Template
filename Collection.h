...
namespace sdds {
   template <typename T>
   class Collection {
      T* m_data = nullptr; //An array of templated objects
      int m_size = 0;
   public:
      Collection(int size = 0);
      Collection(const Collection<T>& CP); //copy constructor
      Collection<T>& operator=(const Collection<T>& RO);  //copy assignment
      int size()const;
   ...
   Collection<T>::Collection(const Collection<T>& CP) {
      operator=(CP); // equals to calling the function of copy assignment??
   }
   template <typename T>
